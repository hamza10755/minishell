...
/*   Updated: 2025/04/03 18:29:02 by hamzabillah      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int  change_and_update_dir(char *dir_path, char *old_pwd, t_token *tokens)
{
    char    *new_pwd;
    ...
    if (tokens && tokens->next && tokens->next->value && ft_strncmp(tokens->next->value, "-", 2) == 0)
        printf("%s\n", new_pwd);
    free(old_pwd);
    free(new_pwd);
    return (0);
